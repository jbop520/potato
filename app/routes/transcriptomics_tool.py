...
transcriptomics_tool_bp = Blueprint(
    "transcriptomics_tool_bp",
    __name__,
...
@transcriptomics_tool_bp.route("/", methods=["GET"])
def index():
    """工具主页面：展示所有工具"""
    try:
...
@transcriptomics_tool_bp.route("/use/<int:tool_id>", methods=["GET"])
    ...
    try:
        ...
        if not tool:
            return render_template(
                ...
                back_url="/transcriptomics_tool_bp/"  # 返回主页链接
            )
        ...
        return render_template(
            ...
            back_url="/transcriptomics_tool_bp/"
        )