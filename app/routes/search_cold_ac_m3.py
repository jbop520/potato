...
from pyecharts import options as opts
search_cold_ac_m3_bp = Blueprint("search_cold_ac_m3_bp", __name__)
...
#             if t["route"] != "search_cold_ac_m3_bp.index"
#         ]
#         return related_treatments
#
#     except Exception as e:
...
def get_gene_related_treatments(gene_id):
    ...
    # 需排除的路由代码（核心配置）
    EXCLUDE_ROUTE = "search_cold_ac_m3_bp.index"
...
# 搜索
@search_cold_ac_m3_bp.route("/", methods=["GET"])
def index():
    q = (request.args.get("q") or "").strip()
    ...
    if q:
        ...
        if rows_c804:
            results.append((cfg["COLD_AC_M3_REF_C804"], rows_c804))

        rows_c882 = query_one_table(cfg["COLD_AC_M3_REF_C882"], "Transcript_ID", q)
        if rows_c882:
            results.append((cfg["COLD_AC_M3_REF_C882"], rows_c882))

        rows_c830 = query_one_table(cfg["COLD_AC_M3_REF_C830"], "Transcript_ID", q)
        if rows_c830:
            results.append((cfg["COLD_AC_M3_REF_C830"], rows_c830))

        rows_c454 = query_one_table(cfg["COLD_AC_M3_REF_C454"], "Transcript_ID", q)
        if rows_c454:
            results.append((cfg["COLD_AC_M3_REF_C454"], rows_c454))

        rows_dm = query_one_table(cfg["COLD_AC_M3_REF_DM"], "Transcript_ID", q)
        if rows_dm:
            results.append((cfg["COLD_AC_M3_REF_DM"], rows_dm))

        rows_t206 = query_one_table(cfg["COLD_AC_M3_REF_T206"], "Transcript_ID", q)
        if rows_t206:
            results.append((cfg["COLD_AC_M3_REF_T206"], rows_t206))
    ...
    return render_template(
        "search_cold_ac_m3.html",
        q=q,
        results=results,
        chart_code=chart_code,
        heatmap_code=heatmap_code,
        associated_genes=associated_genes,
        tbl804=cfg["COLD_AC_M3_REF_C804"],
        tbl882=cfg["COLD_AC_M3_REF_C882"],
        tbl830=cfg["COLD_AC_M3_REF_C830"],
        ...
        related_treatments=related_treatments# 传递transcriptomics数据到模板
